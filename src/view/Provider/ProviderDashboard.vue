  ...
  <div class="container-fluid py-3 px-md-4">
      ...
      <div class="col-6 col-lg-3">
        <div class="stat-card card border-0 shadow-sm rounded-4 p-3 h-100">
          <div class="d-flex justify-content-between align-items-start mb-3">
            ...
            <span class="trend-badge trend-down"><i class="bi bi-arrow-down me-1"></i>2%</span>
          </div>
          <div v-if="loading">
            <div class="skel-line" style="width: 60px; height: 28px; border-radius: 8px"></div>
          </div>
    ...
    <div class="row g-3">
      ...
      <div class="col-lg-4 d-flex flex-column gap-3">
        ...
        <div class="card border-0 shadow-sm rounded-4 p-4 bg-navy text-white flex-fill">
          <h5 class="fw-bold mb-4">
            <i class="bi bi-hdd-network-fill me-2" style="color: #ff9a5c"></i>Account Status
          </h5>
          <div class="d-flex justify-content-between mb-3">
            <span class="small opacity-75">Role</span>
            <span
              class="badge"
              style="
          ...
          <div class="d-flex justify-content-between mb-3">
            <span class="small opacity-75">My Rooms</span>
            <span class="fw-bold">{{ myRooms.length }}</span>
          </div>
          <div class="d-flex justify-content-between mb-3">
            <span class="small opacity-75">Avg. Price</span>
            <span class="fw-bold">{{ avgPrice }}</span>
          </div>
          <div class="d-flex justify-content-between mb-3">
            <span class="small opacity-75">Pending Actions</span>
            <span class="fw-bold" style="color: #fbbf24">{{ pendingBookings + pendingRents }}</span>
          </div>
          <div class="d-flex justify-content-between mb-4">
            <span class="small opacity-75">Total Approved</span>
...
const myRooms = computed(() => {
  ...
  return (roomStore.rooms || []).filter(
    (r) => String(r.user_id) === String(uid) || String(r.user?.id) === String(uid),
  )
})
...
async function refreshAll() {
  ...
  try {
    ...
    const [bookRes, rentRes] = await Promise.all([
      ...
      api.get('/profile/rent', { params: { page: 1, per_page: 50 } }),
    ])

    bookings.value = bookRes.data?.data || bookRes.data || []
    rents.value = rentRes.data?.data?.data || rentRes.data?.data || rentRes.data || []
